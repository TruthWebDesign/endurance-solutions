<section class="hero flex items-center justify-center px-15px h-300px sm:h-440px xlg:h-530px 16inch:h-770px bg-no-repeat bg-20% bg-left lg:bg-right  bg-cover" style=" background-color: #F5F5F5; background-image: url('<?php the_field('image_background_hero')?>');">
  <div class="w-full max-w-1270px ">
    ...
    <div class="flex mt-30px gap-3">
      <a class="bg-brown-primary hover:bg-light-brown-gray duration-300  flex items-center text-white font-roboto-slab-medium  uppercase pl-59px pr-59px py-4px sm:py-13px rounded-40px text-tn sm:text-sm " href="<?php the_field('button_first_link_hero')?>"><?php the_field('button_first_text_hero')?></a>
      <a class=" text-navy-blue font-roboto-slab-medium border-2 hover:text-light-brown-gray  hover:border-light-brown-gray duration-300  border-navy-blue uppercase pl-52px pr-52px py-4px sm:py-13px rounded-40px text-tn sm:text-sm " href="<?php the_field('button_second_link')?>"><?php the_field('button_second_text_hero')?></a>
    </div>
  </div>
</section>
