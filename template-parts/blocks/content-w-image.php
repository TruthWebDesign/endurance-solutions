<?php if(get_field('style_content_image')):?>
<section class="flex flex-col xslg:flex-row justify-between items-center max-w-1300px m-auto px-15px  py-30px sm:py-60px gap-3 sm:gap-7">
  <div class="left">
  ...
  <p class="xslg:max-w-542px pb-15px sm:pb-25px 2xlg:pb-34px  font-roboto-slab-regular text-gray-navy text-sm 2xlg:text-rg"><?php the_field('paragraph_content_image');?></p>
  <?php if( have_rows('list_content_image') ):?>
    <ul class="flex flex-col gap-14px pb-15px sm:pb-25px 2xlg:pb-34px text-gray-navy text-sm 2xlg:text-md">
     <?php while( have_rows('list_content_image') ) : the_row();?>
      <li class="flex gap-2 sm:gap-18px tems-center"><span class="mt-6px"><?php echo do_shortcode( '[icon name="valid"]' )?></span><?php the_sub_field('item')?></li>
      <?php endwhile;?>
    </ul> 
  <?php endif;?>
